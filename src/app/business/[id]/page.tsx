...
import { Enterprise, Review, BusinessService } from '@/app/lib/types';
...
import { saveBusinessToCache, getCachedBusiness } from '@/lib/offline-db';
...
const DetailMap = dynamic(() => import('@/components/business/DetailMap'), { 
  ssr: false,
  loading: () => <div className="w-full h-[200px] bg-muted animate-pulse rounded-xl border border-[#E5E5E1]" />
});
...
export default function BusinessDetailPage() {
  ...
  const [offlineData, setOfflineData] = useState<Enterprise | null>(null);
  ...
  const { data: remoteBusiness, isLoading: isBusinessLoading } = useDoc<Enterprise>(businessRef);

  const business = remoteBusiness || offlineData;

  useEffect(() => {
    if (id) {
      getCachedBusiness(id as string).then(cached => {
  ...
  useEffect(() => {
    if (remoteBusiness) {
      saveBusinessToCache(remoteBusiness);
      document.title = `${remoteBusiness.businessName || remoteBusiness.name} | Cameroon Zoom`;
    }
  }, [remoteBusiness]);
  ...
  if (isBusinessLoading && !business) return <div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin text-secondary" /></div>;
  ...
  return (
    <div className="min-h-screen bg-white font-body text-secondary pb-24">
      ...
      <div className="container mx-auto px-4 pt-6 max-w-7xl">
        ...
        <div className="bg-white border border-[#E5E5E1] rounded-2xl p-6 sm:p-10 mb-10 shadow-sm relative overflow-hidden">
          <div className="flex flex-col lg:flex-row gap-10">
            <div 
              className={cn(
                "relative w-32 h-32 sm:w-48 sm:h-48 rounded-2xl border border-[#E5E5E1] bg-white shrink-0 overflow-hidden shadow-sm",
                "cursor-pointer hover:scale-[1.02] transition-transform active:scale-95"
              )}
              onClick={() => setPreviewImg(business.logo || 'https://picsum.photos/seed/inst-build/400/400')}
            >
              {business.logo ? (
                <Image src={business.logo} alt="Logo" fill className="object-contain p-4" unoptimized priority />
              ) : (
                <div className="flex items-center justify-center h-full opacity-10"><Building2 className="w-12 h-12" /></div>
              )}
            </div>
            ...
            <div className="flex-grow space-y-6">
              <div className="flex items-start justify-between flex-wrap gap-4">
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    {business.isVerified && (
                      <div className="bg-primary text-white text-[8px] font-black uppercase px-2 py-1 shadow-sm rounded-sm">
                        Verified
                      </div>
                    )}
                  </div>
                  <div className="flex items-center gap-3">
                    <Badge variant="outline" className="text-primary border-[#E5E5E1] font-bold text-[9px] uppercase h-7 px-4 rounded-xl">
                      {business.category}
                    </Badge>
                    <div className="flex items-center gap-1.5 bg-muted/50 px-3 py-1 rounded-xl border border-[#E5E5E1]">
                ...
                <div className="flex items-center gap-2">
                  <Button onClick={() => setShowShareModal(true)} variant="outline" size="icon" className="h-10 w-10 border border-[#E5E5E1] rounded-full">
              ...
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                  <div className="bg-primary text-white px-2.5 py-1 rounded-md text-sm font-bold flex items-center gap-1.5">
                    {(business.averageRating || 0).toFixed(1)} <Star className="w-3.5 h-3.5 fill-current" />
                  </div>
                  <span className="text-[11px] font-bold text-muted-foreground uppercase">({business.totalReviews || 0} Reviews)</span>
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                <Button onClick={() => window.open(`tel:${business.phoneNumber}`)} className="h-12 bg-secondary text-white px-8 font-bold uppercase text-[10px] tracking-widest rounded-xl hover:bg-primary transition-all">
                  <Phone className="w-4 h-4 mr-2" /> Call Now
                </Button>
                <Button onClick={() => window.open(`https://wa.me/${(business.whatsapp || business.phoneNumber).replace(/\s+/g, '')}`)} variant="outline" className="h-12 border border-[#25D366] text-[#25D366] px-8 font-bold uppercase text-[10px] tracking-widest rounded-xl hover:bg-[#25D366] hover:text-white transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-4 relative">
            <section className="bg-white border border-[#E5E5E1] rounded-2xl p-8 space-y-8 shadow-sm lg:sticky lg:top-6">
              <div className="space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  {business.latitude && business.longitude && (
                    <div className="h-[200px] w-full rounded-xl border border-[#E5E5E1] overflow-hidden relative shadow-sm">
                      <DetailMap lat={business.latitude} lng={business.longitude} businessName={business.businessName || business.name} />
                    </div>
                  )}

                  <div className="grid gap-3">
                    <Button onClick={() => window.open(`https://www.google.com/maps/dir/?api=1&destination=${business.latitude},${business.longitude}`, '_blank')} className="w-full h-12 bg-secondary text-white font-bold uppercase text-[10px] tracking-widest rounded-xl hover:bg-primary transition-all">
                    ...
                    <div className="space-y-4 pt-6 border-t border-[#E5E5E1]">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center gap-3 group cursor-pointer" onClick={() => window.open(`tel:${business.phoneNumber}`)}>
                          <div className="w-9 h-9 rounded-lg bg-muted/50 border border-[#E5E5E1] flex items-center justify-center group-hover:border-primary transition-all">
                    ...
                    {!business.isVerified && (
                      <div className="pt-6 mt-4 border-t border-[#E5E5E1]">
                        <Button 
                          ...
                          variant="outline"
                          className="w-full h-12 border border-secondary text-secondary font-bold uppercase text-[10px] tracking-widest rounded-xl hover:bg-muted/20"
                        >
                          <BadgeCheck className="w-4 h-4 mr-2" /> Claim Business
                        </Button>
                      </div>
                    )}
                  </div>
              ...
              <div className="space-y-6 pt-2 border-t border-[#E5E5E1]">
                ...
                <div className={cn("p-4 bg-muted/30 border border-[#E5E5E1] rounded-xl overflow-hidden transition-all duration-300", !showFullHours && "max-h-[60px]")}>
                  <p className="text-[12px] font-bold uppercase whitespace-pre-line leading-relaxed text-secondary/70">{business.hours}</p>
                </div>
              </div>
              ...
              <div className="space-y-6 pt-8 border-t border-[#E5E5E1]">
                ...
                <form onSubmit={handleEnquiry} className="space-y-4">
                  <Input required placeholder="My Name" className="border border-[#E5E5E1] h-11 text-xs rounded-xl font-bold" value={enquiry.name} onChange={e => setEnquiry({...enquiry, name: e.target.value})} />
                  <Input required type="email" placeholder="My Email" className="border border-[#E5E5E1] h-11 text-xs rounded-xl font-bold" value={enquiry.email} onChange={e => setEnquiry({...enquiry, email: e.target.value})} />
                  <Textarea required placeholder="My Message..." className="border border-[#E5E5E1] min-h-[100px] text-xs rounded-xl italic font-bold" value={enquiry.message} onChange={e => setEnquiry({...enquiry, message: e.target.value})} />
                  <Button disabled={isSendingEnquiry} type="submit" className="w-full bg-secondary text-white font-bold uppercase text-[10px] h-12 rounded-xl shadow-md hover:bg-primary transition-all">
          ...
          <div className="lg:col-span-8 space-y-10">
            <section className="bg-white border border-[#E5E5E1] rounded-2xl p-8 space-y-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-6 border-t border-[#E5E5E1]">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-muted/50 border border-[#E5E5E1] flex items-center justify-center">
                    <BadgeCheck className="w-5 h-5 text-primary" />
                  </div>
                  <div>
                    <p className="text-[9px] font-bold uppercase text-muted-foreground tracking-widest">Boss Name</p>
            ...
            <section className="bg-white border border-[#E5E5E1] rounded-2xl p-8 space-y-10 shadow-sm">
              <div className="flex items-center justify-between border-b border-[#E5E5E1] pb-6">
                ...
                <Button onClick={() => setShowReviewModal(true)} className="h-9 bg-primary text-white font-bold uppercase text-[9px] tracking-widest rounded-xl shadow-sm">Add Review</Button>
              </div>
